...
### 4.3 分层 content hash 与 acquisition_manifest_hash

- `acquisition_bundle_content_hash`：完整规范数据包，包含全部已下载流与当前 contract rules；旧字段 `dataset_content_hash` 仅作为该值的兼容别名。
- `strategy_data_content_hash`：只包含每个标的的 `trade_1m`、原生 `trade_4h` 和原生 `trade_1d`。
- `execution_data_content_hash`：只包含每个标的的 `trade_1m`、`mark_1m` 和真实 funding records。
- `audit_data_content_hash`：只包含可选 `index_1m`；即使未请求 Index，也使用版本化空审计包生成确定性哈希。
- `contract_rule_content_hash`：只包含去除采集时间和 source hash 后的当前 contract-rule Canonical 内容。
- 每个分层哈希都绑定冻结的 bundle version、数据集名称和各自 dataset content hash；均与下载重试次数、下载时间和页边界无关。
...
- `computational_experiment_id`：接收调用者显式声明的版本化 `content_dependency_hashes`、实验 scope、样本区间、策略版本、执行版本、成本版本、代码 commit 和依赖锁版本。它不得包含 acquisition manifest hash、acquisition run ID、下载时间、分页或重试信息。
- `candidate` scope 只允许依赖 `strategy_data@STRATEGY_DATA_CONTENT_V1`；不得绑定可选 Index、当前 contract rules、execution bundle 或 audit bundle。

重新下载得到相同 Canonical 数据时，相应分层 content hash 和使用相同显式依赖的 `computational_experiment_id` 必须相同；采集过程不同可产生不同 `acquisition_manifest_hash` 与 `acquisition_run_id`。采集哈希和采集 ID 不得进入 Candidate、FillEvent、账本或策略计算的确定性 ID。
...
`computational_experiment_id` 仅由显式版本化 content dependency hashes、experiment scope、样本区间、策略版本、执行版本、成本版本、代码 commit 和依赖锁版本生成。Candidate scope 只接受 strategy-data dependency；`acquisition_manifest_hash`、`acquisition_run_id`、下载时间、分页和重试不参与。相同 computational experiment ID 的结果必须逐字节一致。
